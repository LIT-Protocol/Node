...
pub(crate) async fn check_access_control_conditions(
    ...
    let auth_sigs = {
        match auth_sig_item {
            AuthSigItem::Multiple(multiple_auth_sigs) => {
                ...
                mult_auth_sigs
                    .borrow_mut()
                    .validate_all_and_extract_wallet_sigs(requested_lit_resource_ability, &cfg)
                    .await?;
    ...
    let res = check_condition_group(conditions, &auth_sigs, cfg, request_id).await?;
    Ok(res)
}
    ...
    for condition_item in conditions {
        match condition_item {
            UnifiedAccessControlConditionItem::Condition(condition) => {
                results.push(check_condition(condition, auth_sigs, cfg.clone(), request_id).await?);
            }
            ...
            UnifiedAccessControlConditionItem::Group(group) => {
                results
                    .push(check_condition_group(group, auth_sigs, cfg.clone(), request_id).await?);
            }
        }
    }
...
async fn check_condition(
    ...
    let condition_check = match condition {
        UnifiedAccessControlCondition::JsonAccessControlCondition(access_control_condition) => {
            ...
            super::check_condition(access_control_condition, &auth_sig, cfg, request_id).await?
        }
        UnifiedAccessControlCondition::EVMContractCondition(evm_contract_condition) => {
            ...
            evm_contract::check_condition(evm_contract_condition, &auth_sig).await?
        }
        UnifiedAccessControlCondition::SolRpcCondition(sol_rpc_condition_version) => {
            ...
            sol_rpc::check_condition(sol_rpc_condition_version, &auth_sig).await?
        }
        UnifiedAccessControlCondition::CosmosCondition(cosmos_condition) => {
            ...
            cosmos::check_condition(cosmos_condition, auth_sigs).await?
        }
    };
