...
pub(crate) async fn check_access_control_conditions(
    ...
    match validate_res {
        Ok(valid_auth_sig) => {
            ...
            let conditions_met =
                check_condition_group(&fixed_conditions, &valid_auth_sig, cfg, request_id).await?;
            Ok(conditions_met)
        }
        Err(_) => Ok(false),
    }
    ...
    for condition_item in conditions {
        match condition_item {
            AccessControlConditionItem::Condition(condition) => {
                results.push(check_condition(condition, auth_sig, cfg.clone(), request_id).await?);
            }
            ...
            AccessControlConditionItem::Group(group) => {
                results
                    .push(check_condition_group(group, auth_sig, cfg.clone(), request_id).await?);
            }
        }
    }
...
where
    C: AsRef<str>,
{
    let url = rpc::rpc_url(chain.as_ref(), 0).map_err(|e| {
        blockchain_err_code(
            e,
...
async fn check_condition(
    ...
    if !condition.standard_contract_type.is_empty() {
        if condition.standard_contract_type == "POAP" {
            ...
            return check_condition_via_lit_action(condition, auth_sig, cfg, request_id).await;
            ...
            return check_condition_via_contract_call(&web3, condition, auth_sig).await;
        }
    } else if condition.method.is_empty() {
        return check_condition_via_signature(&web3, condition, auth_sig).await;
    } else {
        return check_condition_via_rpc_method(&web3, condition, auth_sig).await;
    }
}
...
async fn check_condition_via_signature(
    ...
    let subbed_param = substitute_special_params(&condition.parameters[0], auth_sig).await?;
    ...
    check_return_value_addr(condition, account_address, auth_sig).await
}
...
async fn check_condition_via_rpc_method(
    ...
    if condition.method == "eth_getBalance" {
        let address_to_check = substitute_special_params(&condition.parameters[0], auth_sig)
            .await
...
async fn check_condition_via_lit_action(
    ...
    use crate::{functions, utils::web::get_auth_context};
    use std::error::Error;
    ...
    for param in &condition.parameters {
        let subbed_param = substitute_special_params(param, auth_sig).await?;
    ...
    spawner.spawn(functions::spawner::Task::ExecuteJs(
        code_to_run,
        Some(ipfs_id),
        auth_sig.clone(),
        None,
    ...
    let execution_result = response.await.map_err(|e| {
        unexpected_err_code(
            e,
            EC::NodeJsExecutionError,
            Some("Error talking to JS runtime".into()),
        )
    })?;
    ...
    if let Some(err) = rust_js_comms.error {
        println!("Error executing JS generated in Rust bindings: {:?}", err);
        return Err(unexpected_err_code(
            err,
            EC::NodeJsExecutionError,
            Some("Error executing JS generated in Rust bindings".into()),
        ));
    }

    let returned_value = rust_js_comms.response;

    check_return_value_str(condition, returned_value)
}
...
async fn check_condition_via_contract_call(
    ...
    if condition.standard_contract_type == "PKPPermissions" {
        if condition.method == "isPermittedAction" || condition.method == "isPermittedAddress" {
            ...
            if condition.method == "isPermittedAction" {
                ...
                let address_to_check = Address::from_slice(
                    &encoding::hex_to_bytes(auth_sig.user_address().await?).map_err(|e| {
        ...
        if condition.method == "balanceOf" {
            let subbed_param =
                substitute_special_params(&condition.parameters[0], auth_sig).await?;
            let account_address =
            ...
            for a in condition.parameters[0].split(',') {
                let subbed_param = substitute_special_params(&a.to_string(), auth_sig).await?;
        ...
        if condition.method == "ownerOf" {
            ...
            return check_return_value_addr(condition, owner, auth_sig).await;
        ...
        if condition.method == "getActiveSubscriptionCount" {
            let subbed_param = substitute_special_params(&condition.parameters[0], auth_sig)
                .await
                .map_err(|e| validation_err_code(e, EC::NodeInvalidSIWESpecialParam, None))?;
            ...
            let subbed_param = substitute_special_params(&condition.parameters[1], auth_sig)
                .await
                .map_err(|e| validation_err_code(e, EC::NodeInvalidSIWESpecialParam, None))?;
...
pub async fn substitute_special_params(param: &String, auth_sig: &JsonAuthSig) -> Result<String> {
    if param == ":userAddress" {
        return auth_sig.user_address().await;
