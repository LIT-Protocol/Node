...
pub async fn check_access_control_conditions(
    ...
    match validate_solana_auth_sig(auth_sig) {
        Ok(sig_is_valid) => {
            ...
            match sig_is_valid {
                true => check_condition_group(conditions, auth_sig).await,
                false => Ok(false),
            }
        }
        Err(err) => {
...
pub async fn check_condition(
    ...
    for param in condition.params.clone() {
        match param {
            serde_json::Value::String(str) => {
                let subbed_param = super::substitute_special_params(&str, auth_sig)
                    .await
                ...
                if p.contains("pubkey:") {
                    return match Pubkey::from_str(&p[7..]) {
                        ...
                        Err(e) => {
                            ...
                            warn!(error = as_error!(e); "Could not get pubkey from pda_params");
                            None
                        }
                    };
                }
    ...
    if condition.method == "balanceOfMetaplexCollection" {
        ...
        return check_balance_of_token(&condition, auth_sig).await;
    } else if condition.method == "getPDA" {
        return check_return_value_string(&condition, pda_address, auth_sig).await;
    }
    ...
    check_return_value(&condition, &parsed_response["value"], auth_sig).await
}
...
async fn check_return_value(
    ...
    match value_to_check {
        ...
        Value::String(val) => check_return_value_string(condition, val.to_string(), auth_sig).await,
        _ => {
...
async fn check_return_value_string(
    ...
    let valid_return_value =
        substitute_special_params(&condition.return_value_test.value, auth_sig)
            .await
            .map_err(|e| validation_err_code(e, EC::NodeInvalidSIWESpecialParam, None))?;
...
async fn check_balance_of_token(
    ...
    match resp {
        Ok(parsed_response) => {
            ...
            let result = parsed_response["value"].clone();
            check_return_value(condition, &result, auth_sig).await
        }
        Err(err) => {
...
mod tests {
    ...
    async fn test_check_condition_string() {
        ...
        let check_balance_condition = check_condition(&address_condition, &get_auth_sig()).await;
