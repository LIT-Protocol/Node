...
impl<M: Middleware + 'static> WebauthnAuthMethodVerifier<M> {
    ...
    async fn verify_webauthn_authentication(
        ...
        let blockhash = encoding::bytes_to_hex(challenge.0.as_slice());
...
fn generate_auth_method_id(credential_raw_id: String) -> Bytes {
    Bytes::from(keccak256(format!("{}:lit", credential_raw_id).into_bytes()))
}
...
/// we accept the last 5 eth block hashes as the challenge
/// this checks that the challenge is one of those 5 hashes
pub async fn check_blockhash_challenge<M: Middleware + 'static>(
    ...
    let latest_block_number = rpc_provider.get_block_number().await.map_err(|e| {
        validation_err_code(
            ...
            EC::NodeRpcError,
            Some(
                "failed to get latest block when checking a webauthn blockhash challenge"
                    .to_string(),
            ),
        )
    })?;

    let blockhash_to_check_as_tx_hash =
        TxHash::from_slice(&encoding::hex_to_bytes(blockhash_to_check)?);
    ...
    for i in 0..5 {
        ...
        // check and return early if possible
        if block_hash == blockhash_to_check_as_tx_hash {
            return Ok(true);
        }
    }
...
mod tests {
    ...
    async fn test_check_blockhash_challenge_success() {
        ...
        // NOTE: Requests are retrieved back-to-front (WTF right?)
        assert!(mock.push(block).is_ok());
        assert!(mock.push(U64::from(1)).is_ok());

        let checked =
            check_blockhash_challenge(block_hash, CHAIN_ETHEREUM.to_owned(), Arc::new(provider))
                .await
                .expect("failed to check blockhash challenge");
        assert!(checked);
    }
