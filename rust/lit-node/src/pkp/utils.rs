use crate::{
    ...
    tss::common::{
        key_type::KeyType, storage::any_key_share_exists, tss_state::TssState,
        web::models::SignedMessageShare,
    },
    utils::{
...
use crate::tasks::{
    beaver_manager::models::generate_hash,
...
pub async fn pkp_permissions_is_permitted(
    ...
    if method == "isPermittedAction" {
        ...
        res = contract
            .is_permitted_action(token_id, Bytes::from(ipfs_id))
            .call()
            .await;
...
pub async fn sign_ecdsa(
...
) -> Result<(SignedMessageShare, KeyType)> {
    // auth check
    ...
    let (tweak_preimage, root_pubkeys, key_type) = match tweak_preimage {
        Ok(_) => {
            ...
            let key_type = KeyType::EcdsaCaitSith; // maybe inspect root keys to determine key type?
        ...
        Err(_) => {
            let result = any_key_share_exists(&pubkey).await;
            info!(
