...
impl KeyPersistence<ProjectivePoint> for KeyHelper<ProjectivePoint> {
    ...
    fn secret_from_hex(&self, hex_private_key: &str) -> Result<Scalar> {
        Scalar::from_be_hex(hex_private_key).ok_or(unexpected_err(
            "Failed to convert hex to p256 private key",
            None,
        ))
    }
    ...
    fn secret_from_bytes(&self, private_key: &[u8]) -> Result<Scalar> {
        ...
        Scalar::from_be_bytes(&private_key[..32]).ok_or(unexpected_err(
            "Failed to convert bytes to p256 private key",
            None,
        ))
    }

    fn pk_from_hex(&self, hex_public_key: &str) -> Result<ProjectivePoint> {
        p256::ProjectivePoint::from_compressed_hex(hex_public_key).ok_or(unexpected_err(
            "Failed to convert hex to p256 public key",
            None,
        ))
    }
    ...
    fn pk_from_bytes(&self, public_key: &[u8]) -> Result<ProjectivePoint> {
        ...
        p256::ProjectivePoint::from_compressed(&public_key[..33]).ok_or(unexpected_err(
            "Failed to convert bytes to p256 public key",
            None,
        ))
    }
}
