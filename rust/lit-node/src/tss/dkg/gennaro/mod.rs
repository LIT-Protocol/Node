...
impl<G: Group + GroupEncoding + Default> GennaroDkg<G> {
    ...
    // Return the old peer_id(s) of all the Survivors as share_ids. Also return the index of the node in share_ids.
    // These values are used to construct the secret for the SecretParticipant::with_secret()
    fn get_share_ids_and_my_index(
        ...
        let next_peers_set = BTreeSet::from_iter(next_peers);
        for peer in current_peers.iter() {
            if next_peers_set.contains(peer) {
                survivors.push(peer.to_string());
            }
        }

        let mut share_ids = Vec::new();
        let mut share_id_index = 0;
        let mut my_share_index = 0;
        for (i, peer) in current_peers.iter().enumerate() {
            if share_id_index == survivors.len() {
                break;
            }
            if my_address == peer {
                my_share_index = share_id_index;
            }
            if &survivors[share_id_index] == peer {
                share_ids.push(G::Scalar::from((i + 1) as u64));
                share_id_index += 1;
            }
        }
    ...
    pub fn init_participant(
        ...
        match mode {
            ...
            Mode::RefreshPeer => {
                let (my_share_index, share_ids) = Self::get_share_ids_and_my_index(
                    &current_peers.all_addresses(),
                    &next_peers.all_addresses(),
                    my_address,
                );
