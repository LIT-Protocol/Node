...
use super::{
    models::{EcdsaKeyShare, ProtocolTransactionParams},
    CsEcdsaState,
};
use crate::{
...
use elliptic_curve::{group::prime::PrimeCurveAffine, CurveArithmetic, ScalarPrimitive};
...
impl CsEcdsaState {
    ...
    pub async fn get_hd_key_threshold(
        ...
        let keyshare =
            read_key_share_from_disk::<EcdsaKeyShare<Secp256k1>>(hd_root_key, share_index, epoch)
                .await
                .map_err(|e| {
                    unexpected_err(e, Some("Could not read key share from disk".into()))
                })?;
    ...
    pub async fn get_triple_pair(
        ...
        txn_prefix: &str,
        threshold: u16,
        peer_subset: Vec<PeerSocketAddress>,
        ...
        // todo - I believe 1 is sufficient here.
    ...
    pub async fn presign(
        &self,
        txn_params: ProtocolTransactionParams,
        keyshare: EcdsaKeyShare<k256::Secp256k1>,
        ...
        self.do_presign::<k256::Secp256k1>(txn_params, keyshare, share0, share1, pub0, pub1)
            .await
    }
    ...
    pub async fn generate_hd_key_signature_share(
        ...
        let share_index = txn_params.node_config.share_index;
        ...
        for hd_root_key in hd_root_keys {
            let keyshare = read_key_share_from_disk::<EcdsaKeyShare<Secp256k1>>(
                &hd_root_key,
                share_index,
                epoch,
            )
        ...
        trace!("HD Key Shares length: {:}", hd_key_shares.len());
        ...
        // let private_share = deriver.compute_secret_key(&hd_secret_share);
        let keygen_output = deriver
            .compute_secret_key_share_cait_sith(&hd_key_shares)
            .map_err(|e| unexpected_err(e, Some("Could not compute secret key share".into())))?;

        let private_share = keygen_output.private_share;
        // let derived_pubkey = keygen_output.public_key;
        ...
        let keyshare = EcdsaKeyShare {
            ...
            threshold,
            total_shares,
            txn_prefix: txn_params.txn_prefix.to_string(),
        };

        // let hd_context = Some(RE_RANDOMIZER_CTX);
        self.generate_signature_share(txn_params, keyshare, message, triple_pair)
            .await
    }
    ...
    pub async fn generate_signature_share_from_pubkey(
        ...
        let txn_params =
            ProtocolTransactionParams::from_config(self.protocol_params(txn_prefix).await?);
        let share_index = txn_params.node_config.share_index;
        let epoch = self.state.peer_state.epoch().await;

        let keyshare =
            read_key_share_from_disk::<EcdsaKeyShare<Secp256k1>>(public_key, share_index, epoch)
                .await
                .map_err(|e| {
                    unexpected_err(e, Some("Could not read key share from disk".into()))
                })?;

        self.generate_signature_share(txn_params, keyshare, message, shares)
            .await
    }
    ...
    pub async fn generate_signature_share(
        &self,
        txn_params: ProtocolTransactionParams,
        keyshare: EcdsaKeyShare<Secp256k1>,
        message: &[u8],
        triple_pair: BeaverTriplePair,
    ) -> Result<CsSigshare> {
        ...
        let public_key = keyshare.public_key.to_encoded_point(true);
        let public_key = public_key.as_bytes();

        // used in the case of hd_key derivation -> rerandomize the presignature
        let associated_data = &triple_pair.pub0.big_a.to_encoded_point(true);
        let associated_data = associated_data.as_bytes(); // safe to re-use as it is being hashed.
        ...
        // this being commented out means that we do raw signing - we do not do a hash before signing.
        // and we assume the message has already been hashed
    ...
    // #[instrument(skip_all, fields(txn_prefix = txn_prefix))]
    // pub async fn generate_triple_pairs(
    //     &self,
    //     peers: Option<Vec<PeerSocketAddress>>,
    //     txn_prefix: String,
    //     quantity: u16,
    // ) -> Result<Vec<BeaverTriplePair>> {
    //     let mut v = Vec::new();

    //     for i in 0..quantity {
    //         let txn_prefix = format!("TRPBATCH{}.{}", i, txn_prefix);
    //         let t = self.generate_triple_pair(peers.clone(), txn_prefix);
    //         v.push(t);
    //     }
    //     let result = join_all(v).await;
    //     info!("Got all triple pairs for batch {}", txn_prefix);

    //     let mut triple_pairs = Vec::new();
    //     for r in result {
    //         let triple_pair =
    //             r.map_err(|e| unexpected_err(e, Some("Could not process triple pair".into())))?;
    //         triple_pairs.push(triple_pair);
    //     }
    ...
    pub async fn generate_triple_pair(
        ...
        let start = std::time::Instant::now();
        for i in 0..2 {
            let dkg_triples = format!("TRP{}.{}", i, txn_prefix);
            let txn_params = ProtocolTransactionParams::from_config(
                self.protocol_params_with_peers(peers.clone(), &dkg_triples)
                    .await?,
            );
        ...
        let (p0, (share0, pub0), peers0) = match result0 {
            Some(Ok(r)) => {
                r.map_err(|e| unexpected_err(e, Some("Could not process triples0".into())))?
            }
            Some(Err(e)) => {
                return Err(unexpected_err(e, Some("Could not process triples0".into())))
            }
            None => return Err(unexpected_err("Could not process triples0", None)),
        };
        ...
        let peers = match peers {
            Some(p) => p,
            None => self.state.peer_state.peer_socket_addresses().await?,
        };
