...
pub async fn get_auth_context(
    ...
    if let Some(auth_sig) = auth_sig {
        ...
        let validate_res = auth_sig
            .validate_and_get_wallet_sig(&lit_action_resource.execution_ability(), &None, &cfg)
            .await
            .map_err(|e| {
        ...
        match validate_res {
            Ok(valid_auth_sig) => {
                ...
                verified_wallet_address = Some(auth_sig.user_address().await.map_err(|e| {
                    validation_err_code(e, EC::NodeInvalidAuthSig, Some("Invalid AuthSig".into()))
                })?);
            }
            Err(e) => {
...
pub fn pubkey_to_token_id(pubkey: &str) -> Result<String> {
    ...
    let token_id = encoding::bytes_to_hex(ethers::utils::keccak256(&pubkey_bytes[1..65]));
    Ok(token_id)
}
