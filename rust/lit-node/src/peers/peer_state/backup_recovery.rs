...
use super::{super::PeerState, models::PeerSocketAddress};
...
impl PeerState {
    ...
    pub async fn get_recovery_dkg_peers(&self) -> Result<Vec<PeerSocketAddress>> {
        ...
        // Map the peer addresses to `PeerSocketAddress` by fetching their `Validator` struct
        let mut recovery_peer_socket_addresses: Vec<PeerSocketAddress> = Vec::new();
        for (index, staker_address) in recovery_peer_addresses.iter().enumerate() {
            ...
            recovery_peer_socket_addresses.push(PeerSocketAddress {
                address: get_web_addr_from_chain_info(validator.ip, validator.port),
                share_index: index as u32, // Every peer has the same index based on the Backup party member mapping on-chain which is an array
                kicked: false,
                key_hash: 0, // Not used for Recovery DKG
            });
        }

        Ok(recovery_peer_socket_addresses)
    }
    ...
    pub async fn register_recovery_keys(&self, recovery_root_keys: Vec<DKGRootKey>) {
        ...
        for recovery_key in recovery_root_keys {
            ...
            recovery_keys.push(RecoveryKey {
                ...
                key_type: recovery_key.key_type.into(),
            });
        }
