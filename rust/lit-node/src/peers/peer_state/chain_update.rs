...
impl PeerState {
    ...
    pub async fn rpc_signal_ready_for_next_epoch(&self, epoch_number: U256) -> Result<()> {
        ...
        match tx.await.map_err(|e| blockchain_err(e, None))? {
            ...
            None => {
                error!("Failed to get transaction receipt for signal ready for next epoch, but the transaction might still have worked. Returning success.");
    ...
    pub async fn rpc_lock_validators_for_next_epoch(&self) {
        ...
        #[cfg(not(feature = "testing"))]
        {
            ...
            let func = self
                ...
                .gas_price(1000000); // this is 0.001 gwei
