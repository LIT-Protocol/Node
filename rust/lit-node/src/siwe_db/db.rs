use crate::error::{conversion_err_code, unexpected_err, unexpected_err_code, Result, EC};
...
use crate::siwe_db::{
    rpc::fetch_block_from_hash,
    utils::{check_block_timestamp_validity, make_timestamp_siwe_compatible},
};

const INDEXER_URL: &str = "http://block-indexer.litgateway.com/get_all_valid_blocks";

#[derive(Clone, Debug, serde::Deserialize)]
struct EthBlockhashInfo {
    blockhash: String,
    timestamp: String,
    block_number: usize,
}
...
fn db_batch_write(conn: &mut Connection, block_records: &Vec<EthBlockhashInfo>) -> Result<()> {
    let tx = conn
        .transaction()
...
pub async fn init_fill_db(port: u16, quit_rx: mpsc::Receiver<bool>) -> Result<()> {
    let blocks: Vec<EthBlockhashInfo> = reqwest::get(INDEXER_URL)
        .await
        .map_err(|e| {
            unexpected_err(
                e,
...
async fn fetch_and_store_block_info<M: Middleware + 'static>(
    ...
    let (block_timestamp, block_number) = fetch_block_from_hash(block_hash, rpc_provider).await?;
    ...
    conn.execute(
        ...
        params![block_hash, &block_timestamp, block_number,],
    )
    .map_err(|e| unexpected_err_code(e, EC::NodeSystemFault, None))?;

    Ok((
        block_hash.to_string(),
        make_timestamp_siwe_compatible(&block_timestamp)?,
    ))
}
...
pub async fn retrieve_and_store_blockhash<M: Middleware + 'static>(
    ...
    let query_res = conn.query_row(
        "SELECT blockhash, timestamp FROM blockhash_timestamp WHERE blockhash=?",
        [&block_hash],
        |row| Ok((row.get::<_, String>(0), row.get::<_, String>(1))),
    );

    match query_res {
        Ok((block_hash, block_timestamp)) => {
            let block_hash = match block_hash {
                Ok(block_hash) => block_hash,
                Err(e) => {
                    return Err(unexpected_err(
                        e,
                        Some("Error getting block_hash from the query result".to_owned()),
                    ));
                }
            };

            let block_timestamp = match block_timestamp {
                Ok(block_timestamp) => block_timestamp,
                Err(e) => {
                    return Err(unexpected_err(
                        e,
                        Some("Error getting block_timestamp from the query result".to_owned()),
                    ));
                }
            };

            check_block_timestamp_validity(&block_timestamp)?;
...
mod siwe_db_tests {
    ...
    use crate::{
        siwe_db::{
            db::{
                ...
                init_fill_db, retrieve_and_store_blockhash, EthBlockhashInfo,
            },
            utils::{convert_siwe_timestamp_to_utc_datetime, MAX_TIMESTAMP_VALIDITY_DAYS},
        },
        utils::encoding,
    };
    ...
    async fn test_fetch_and_store_block_info() {
        ...
        let (hash, timestamp) = fetch_and_store_block_info(block_hash, port, provider)
            .await
            .unwrap();
        let utc_timestamp = convert_siwe_timestamp_to_utc_datetime(timestamp)
            .unwrap()
            .timestamp();

        // Returned hash & timestamp is stored in the DB
        assert_eq!(&hash, &block_hash);
        assert_eq!(&now, &utc_timestamp);
    ...
    async fn test_retrieve_and_store_blockhash() {
        ...
        let (hash, timestamp) =
            retrieve_and_store_blockhash(block_hash1.clone(), port, provider.clone())
                .await
                .unwrap();
        let utc_timestamp = convert_siwe_timestamp_to_utc_datetime(timestamp)
            .unwrap()
        ...
        assert_eq!(&block_timestamp, &utc_timestamp.to_string());

        // 1. block_hash1 already exists in the DB
        // 2. block_timestamp_expired is invalid
        assert!(
            retrieve_and_store_blockhash(block_hash2.clone(), port, provider.clone())
                .await
                .is_err()
        );
        ...
        // 2. block_timestamp is valid
        let (hash, timestamp) =
            retrieve_and_store_blockhash(block_hash.to_string(), port, provider.clone())
                .await
                .unwrap();
        let utc_timestamp = convert_siwe_timestamp_to_utc_datetime(timestamp)
            .unwrap()
        ...
        assert_eq!(&hash, block_hash);
        assert_eq!(&now.to_string(), &utc_timestamp.to_string());
    ...
    fn add_blocks_to_db(
        conn: &mut Connection,
    ) -> (String, String, String, String, usize, Vec<EthBlockhashInfo>) {
        let block_hash1 =
        ...
        let block_timestamp_expired = (Utc::now() - Duration::days(MAX_TIMESTAMP_VALIDITY_DAYS))
            ...
            .to_string();
        let block_number: usize = 14402566;
        let block1 = EthBlockhashInfo {
            blockhash: block_hash1.clone(),
            timestamp: block_timestamp.clone(),
            block_number,
        };
        let block2 = EthBlockhashInfo {
            blockhash: block_hash2.clone(),
            timestamp: block_timestamp_expired.clone(),
            block_number,
        };

        let mut records: Vec<EthBlockhashInfo> = Vec::new();
