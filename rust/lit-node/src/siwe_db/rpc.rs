        ...
        loop {
            tokio::select! {
                ...
                _ = block_interval.tick() => {
                  ...
                  let (blockhash, timestamp) = match fetch_latest_blockhash(rpc_provider).await {
                    Ok((blockhash, timestamp)) => (blockhash, timestamp),
                    Err(e) => {
                    ...
                    *write_blockhash = format!("0x{}", hex::encode(blockhash.to_fixed_bytes()));
...
pub async fn fetch_block_from_hash<M: Middleware + 'static>(
    ...
    Ok((block_timestamp.to_string(), block_number.as_usize()))
}

async fn fetch_latest_blockhash<M: Middleware + 'static>(rpc_provider: M) -> Result<(H256, U256)> {
...
mod siwe_block_fetch_tests {
    ...
    async fn test_fetch_block_from_hash() {
        ...
        let row = row.unwrap();
        assert_eq!(row.0, block_timestamp.to_string());
        assert_eq!(row.1, block_number);
