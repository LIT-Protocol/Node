...
use lit_blockchain::contracts::staking::{AddressMapping, Version};
...
pub struct GenericConfig {
    ...
    pub complaint_tolerance: u64,
    pub complaint_interval_secs: u64,
...
impl ChainDataConfigManager {
    pub fn new(config: ReloadableLitConfig) -> Self {
        Self {
            ...
            peers_for_current_epoch: RwLock::new(PeersForEpoch {
                validators: Vec::new(),
                epoch_id: "".to_string(),
                epoch_number: 0,
            }),
            peers_for_next_epoch: RwLock::new(PeersForEpoch {
                validators: Vec::new(),
                epoch_id: "".to_string(),
                epoch_number: 0,
            }),
            ...
            generic_config: RwLock::new(GenericConfig {
                ...
                complaint_tolerance: 999,
                complaint_interval_secs: 60,
                key_types: Vec::new(),
    ...
    pub async fn watch_chain_data_config(&self, mut quit_rx: mpsc::Receiver<bool>) {
        ...
        loop {
            ...
            let res = self.set_generic_config_from_chain().await;
            if let Err(e) = res {
    ...
    pub async fn set_root_keys_from_chain(&self) -> Result<()> {
        ...
        for k in root_keys.into_iter() {
            root_pubkey_cache.push(CachedRootKey {
                ...
                key_type: KeyType::try_from(k.key_type)?,
            })
        }
        Ok(())
    }
    ...
    pub async fn set_peer_and_epoch_data_from_chain(&self) -> Result<()> {
        ...
        peers_for_current_epoch.epoch_number = epoch_number.as_u64();
        ...
        peers_for_next_epoch.epoch_number = epoch_number.as_u64() + 1;

        Ok(())
    }

    pub async fn set_generic_config_from_chain(&self) -> Result<()> {
        trace!("set_generic_config_from_chain()");
        let (config, contract_resolver) = self.get_config_with_resolver()?;
        ...
        let token_reward_per_token_per_epoch = staking_contract_config
            ...
            .as_u64();
        let complaint_tolerance = staking_contract_config.complaint_tolerance.as_u64();
        let complaint_interval_secs = staking_contract_config.complaint_interval_secs.as_u64();

        let key_types = staking_contract_config
            .key_types
            .iter()
            .map(|k| KeyType::try_from(*k).expect("Key Types in Staking Config should be valid."))
            .collect::<Vec<KeyType>>();
        ...
        match max_concurrent_requests.cmp(&old_max_concurrent_requests) {
            ...
            Greater => {
                let permits_diff = max_concurrent_requests - old_max_concurrent_requests as u64;
                warn!(
            ...
            Less => {
                let permits_diff = old_max_concurrent_requests as u64 - max_concurrent_requests;
                warn!(
        ...
        generic_config.complaint_tolerance = complaint_tolerance;
        generic_config.complaint_interval_secs = complaint_interval_secs;
