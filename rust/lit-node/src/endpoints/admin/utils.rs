...
pub(crate) async fn encrypt_and_tar_backup_keys(
    ...
    let mut path = encrypted_key_path();
    ...
    let ecdsa_shares =
        read_all_backup_from_disk::<EcdsaKeyShare<Secp256k1>>(&node_set_hash).await?;
    ...
    let bls_shares = read_all_backup_from_disk::<BlsKeyShare<G1Projective>>(&node_set_hash).await?;
    for (file_name, share) in bls_shares {
        let backup = GennaroState::generate_backup(*bls_enc_key, &share, bls_blinder, cfg).await?;
        write_to_disk(path.clone(), &file_name, &backup).await?;
    }
...
pub(crate) async fn untar_keys_stream<R: AsyncRead + Unpin>(
    ...
    let encrypted_bls_shares: Vec<(String, VerifiableBackup<Bls12381G1>)> =
        read_encrypted_keys_from_disk(&path).await?;
...
mod test {
    ...
    type EcdsaShare = EcdsaKeyShare<Secp256k1>;
    ...
    async fn test_encrypt_tar_and_untar_backup_keys() {
        ...
        write_backup_to_disk(pubkey, 0, &bls_key, &[])
            .await
            .unwrap();
        write_backup_to_disk(pubkey, 0, &ecdsa_key, &[])
            .await
            .unwrap();
        ...
        assert_eq!(bls_key_private_share, bls_key.private_share);

        let ecdsa_key_private_share = decrypt_ecdsa_share(encrypted_ecdsa_key, &ecdsa_blinder);
        assert_eq!(ecdsa_key_private_share, ecdsa_key.private_share);
    }
