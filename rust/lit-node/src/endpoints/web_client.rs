...
use crate::utils::web::{
    ...
    hash_access_control_conditions, ConcurrencyGuard,
};
...
pub(crate) async fn signing_access_control_condition(
    ...
    with_context(tracing.clone(), async move {
        ...
        timing.insert("hash access control conditions".to_string(), before.elapsed());
        ...
        let (signature_share, share_index) = match cipher_state.sign(message_to_sign.clone().into_bytes(), epoch).await {
        ...
        debug!("POST /web/signing/access_control_condition timing: {:?}", timing);
    ...
    with_context(tracing.clone(), async move {
        ...
        if let Some(code) = &json_execution_request.code {
            let decoded_bytes = match data_encoding::BASE64.decode(code.as_bytes()) {
                ...
                Err(err) => {
                    return conversion_err(err, Some("Your Lit Action code could not be decoded from base64".into()))
                        .add_msg_to_details()
                        .handle();
                }
            };
        ...
        let http_headers = {
            ...
            for h in request_headers.headers.iter() {
                ...
                res.entry(name).and_modify(|e| e.push_str(&format!(", {value}"))).or_insert(value);
            }
            res
        };
        ...
        let mut client = match action_client::ClientBuilder::default()
            ...
            .build()
            .map_err(|e| unexpected_err_code(e, EC::NodeJsExecutionError, Some("Error building action client".into())))
        {
            Ok(client) => client,
            ...
            Err(err) => {
                ...
                match err.kind() {
                    lit_api_core::error::Kind::Timeout => {
                        return timeout_err_code(err, EC::NodeJsTimeoutError, None).handle_with_logs(logs);
                    },
                    lit_api_core::error::Kind::MemoryLimit => {
                        return memory_limit_err_code(err, EC::NodeJsMemoryLimitError, None).handle_with_logs(logs);
                    },
                    lit_api_core::error::Kind::Connect => {
                        return connect_err_code(err, EC::NodeJsConnectionError, None).handle_with_logs(logs);
                    },
                    _ => {}
                }
                if let Some(source_err) = err.source() {
                    return status::Custom(
                        Status::BadRequest, json!({"success": false, "error": source_err.to_string(), "logs": logs}),
                    );
                }
                return unexpected_err_code(err, EC::NodeJsExecutionError, Some("Error executing JS".into())).handle_with_logs(logs);
            }
        };
    ...
    with_context(tracing.clone(), async move {
        ...
        if let Some(statement) = &parsed_siwe.statement {
            if statement.contains(LIT_RESOURCE_PREFIX_RAC) {
                return validation_err_code("Can't define Auth Context resources in capability", EC::NodeInvalidAuthContextResource, None)
                .add_msg_to_details()
                .handle();
            }
        }
                ...
                let execution_state = match execution_result {
                    ...
                    Err(err) => {
                        ...
                        match err.kind() {
                            ...
                            lit_api_core::error::Kind::MemoryLimit => {
                                return memory_limit_err_code(
                                    err,
                                    EC::NodeJsMemoryLimitError,
                                    None,
                                )
                                .handle_with_logs(logs);
                            }
        ...
        let before = std::time::Instant::now();
        let block =
            match db::retrieve_and_store_blockhash(parsed_siwe.nonce.clone(), port, rpc_provider)
                .await
            {
        ...
        match &json_sign_session_key_request.curve_type {
                    ...
                    json!(JsonSignSessionKeyResponse {
                        ...
                        signed_data: JsonSignSessionKeyResponseInnerSignedData {
                            session_sig: JsonSignSessionKeyResponseInnerSigShare {
                                ...
                                bigr: result.0.big_r,
                                public_key: encoding::bytes_to_hex(pkp_public_key)
                                    .replace("0x", ""),
            ...
            CurveType::BLS => {
                ...
                let before = std::time::Instant::now();
                let (signature_share, share_index) = match cipher_state.sign(to_sign.clone(), epoch).await
                {
                    Ok(signature_share) => signature_share,
                    Err(e) => {
