...
pub async fn do_share_download_from_rec_dkg(
    ...
    let (bls_share, ecdsa_share) =
        match resolve_key_shares_from_disk(&next_backup_state, index as u16).await {
            Ok(shares) => shares,
            Err(e) => {
                return Err(unexpected_err(
                    e,
    ...
    // ecdsa and bls private shares (scalars)
    let bls_priv_share = bls_share.private_share.to_be_bytes();
    let ecdsa_priv_share = ecdsa_share.private_share.to_be_hex();

    // ecdsa and bls public points (public keys)
    let bls_pub_key = bls_share.public_key.to_compressed();
    let ecdsa_pub_key = ecdsa_share.public_key.to_compressed();
    ...
    Ok(vec![
        ...
        DownloadedShareData {
            ...
            decryption_key_share: ecdsa_priv_share,
...
pub async fn do_delete_share_from_disk(
    ...
    let is_deleted = delete_key_shares_from_disk(&next_backup_state, index as u16).await?;
