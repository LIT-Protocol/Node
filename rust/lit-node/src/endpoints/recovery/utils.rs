...
pub async fn resolve_key_shares_from_disk(
    ...
    share_index: u16,
) -> Result<(BlsKeyShare<G1Projective>, EcdsaKeyShare<Secp256k1>)> {
    let mut bls_share: Option<BlsKeyShare<G1Projective>> = None;
    let mut ecdsa_share: Option<EcdsaKeyShare<Secp256k1>> = None;
    ...
    for key in next_backup_state.registered_recovery_keys.clone() {
        if key.key_type == ethers::types::U256::from(1) {
            ...
            let share: BlsKeyShare<G1Projective> =
                match crate::tss::common::storage::read_key_share_from_disk(
                ...
                .await
                {
                    ...
                    Err(e) => {
                        return Err(unexpected_err(
                            e,
                            Some(
                                "Error while getting next backup state from contract. aborting"
                                    .into(),
                            ),
                        ));
                    }
            ...
            trace!("Attempting to read key share of type: {:?}", key.key_type);
            let share: EcdsaKeyShare<Secp256k1> =
                match crate::tss::common::storage::read_key_share_from_disk(
                    bytes_to_hex(&key.pubkey).as_str(),
                    share_index,
                    BACKUP_KEYSHARE_EPOCH,
