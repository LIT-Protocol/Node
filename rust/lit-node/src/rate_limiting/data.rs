...
use crate::error::{
    blockchain_err_code, parser_err, parser_err_code, unexpected_err_code, validation_err_code,
    Result, Unexpected, EC,
};
...
/// This function checks whether the user has reached the free tier rate limit based
/// ONLY on the usage map, NOT against any of the plugin validators.
///
/// If the free tier has not been reached, this function will add a new entry to the usage map.
pub(super) async fn check_free_tier_reached(
    rate_limit_db: &RateLimitDB,
    user_address: &String,
    free_tier_rate_limit_requests_per_second: f32,
) -> bool {
    // Checking for wallet entry
    let usage_map = rate_limit_db.free_tier_wallet_usage_map.read().await;
    let usage = match usage_map.get(user_address) {
        Some(usage) => usage,
        None => {
            // uh ok, this is a new entry.  time to mutate usage_map.
            drop(usage_map);
            let mut usage_map_writeable = rate_limit_db.free_tier_wallet_usage_map.write().await;
            let new_usage_entries = UsageEntries {
                timestamps: RwLock::new(vec![SystemTime::now()]),
            };
            usage_map_writeable.insert(user_address.to_owned(), new_usage_entries);
            return false;
        }
    };
    ...
    // TODO there's an optimization here where we can grab the read lock instead of write lock
    // and then only grab the write lock if the user is under the rate limit, or if we want
    // to drain the timestamps array to remove the old entires.  like maybe we check if it has over 1000 entries, and then we drain the ones older than the rate limit window.
    let mut timestamps = usage.timestamps.write().await;
    trace!("timestamps: {:?}", timestamps);
    drop_usage_entries_older_than_time(timestamps.as_mut(), window_start);

    // check if we're over the limit by calculating requests per second and comparing to the limit
    let requests_per_second =
        timestamps.len() as f32 / rate_limit_config.default_window_duration_secs.as_secs() as f32;

    debug!(
        "Checking if requests_per_second {} is over limit {}",
        requests_per_second, free_tier_rate_limit_requests_per_second
    );

    if requests_per_second < free_tier_rate_limit_requests_per_second {
...
pub(super) async fn get_free_tier_rate_limit_requests_per_second(
    rate_limit_db: &RateLimitDB,
) -> f32 {
    let rate_limit_config = rate_limit_db
        .chain_data_config_manager
    ...
    rate_limit_config.default_max_requests as f32
        / rate_limit_config.default_window_duration_secs.as_secs() as f32
}

/// This function gets the request rate of the given user address in the free tier ONLY.
pub(super) async fn get_requests_per_second(
    rate_limit_db: &RateLimitDB,
    user_address: &String,
) -> f32 {
    // Checking for wallet entry
    ...
    // prune timestamps older than the rate limit window
    let window_start = SystemTime::now() - rate_limit_config.default_window_duration_secs;
    let drop_before_index: Option<usize> = None;
...
pub async fn get_authorized_rate_limit_nfts(
                    ...
                    if inner_ability.0.clone().into_inner() == *"Auth/Auth".to_string() {
                        ...
                        for map in inner_ability.1.clone().into_iter() {
                            ...
                            if let Some(delegate_to) = map.get("delegate_to") {
                                ...
                                let delegate_to_arr =
                                    delegate_to.as_array().expect("delegate_to is not an array");
                                let user_address_str =
                                    bytes_to_hex(user_address).to_ascii_lowercase();
                                let user_is_delegate = delegate_to_arr.iter().any(|x| {
                                    x.as_str()
                            ...
                            if let Some(uses) = map.get("uses") {
                                let max_uses = uses
                                    .as_str()
                                    .expect("Could not convert uses in delegation recap to string")
                                    .parse::<u64>()
                                    .expect("Could not convert uses in delegation recap to u64");

                                let already_used = rate_limit_db
                                    .delegation_uses_map
                                    .get(&delegation_signature_key)
                                    .unwrap_or(0);
                                debug!("User uses: {} and max uses: {}", already_used, max_uses,);
                            ...
                            if let Some(nft_id) = map.get("nft_id") {
                                ...
                                nfts =
                                    fetch_rate_limit_nfts_by_ids(&nft_id_arr, rate_limit_db, cfg)
                                        .await?
                                        .iter()
                                ...
                                .map(|x| PossiblyDelegatedRateLimitNft {
                                    nft: x.clone().nft,
                                    signature_hash_uses_key: Some(delegation_signature_key.clone()),
                                })
                                .collect::<Vec<PossiblyDelegatedRateLimitNft>>();
                            }
...
pub(super) async fn get_owned_and_authorized_rate_limit_nfts(
    session_sig: &JsonAuthSig,
    user_address: Address,
    authorized_rate_limit_nft_ids: &Vec<String>,
    rate_limit_db: &RateLimitDB,
    cfg: &LitConfig,
) -> Result<Vec<PossiblyDelegatedRateLimitNft>> {
    ...
    // Get authorized NFTs
    let nfts =
        get_authorized_rate_limit_nfts(user_address, session_sig, rate_limit_db, cfg).await?;
    for nft in nfts {
        all_authorized_nfts.insert(nft);
    }

    // Get owned NFTs
    for owned_nft in get_rate_limit_nfts_by_owner(user_address, rate_limit_db, cfg).await? {
        all_authorized_nfts.insert(owned_nft);
    }
...
mod calculate_requests_permitted_from_token_tests {
    ...
    async fn get_test_cases() -> Vec<TestCase> {
        ...
        let cdm = Arc::new(ChainDataConfigManager::new(reloadable_cfg));
    ...
    async fn test_get_oldest_timestamp() {
        ...
        for test_case in test_cases {
            let oldest_timestamp =
                get_oldest_timestamp(&test_case.user_address, &test_case.rate_limit_db).await;
            ...
            TestCase {
                ...
                rate_limit_db: {
                    let rate_limit_db =
                        RateLimitDB::default_with_chain_data_config_manager(cdm.clone());
                    {
                        let mut usage_map = rate_limit_db.free_tier_wallet_usage_map.write().await;
                        usage_map.insert(
                            "123".into(),
                            UsageEntries {
                                timestamps: RwLock::new(vec![]),
                            },
                        );
                    }
                    rate_limit_db
...
#[cfg(test)]
mod check_free_tier_reached_tests {
    use std::{sync::Arc, time::SystemTime};
    ...
    use super::check_free_tier_reached;
    ...
    #[tokio::test]
    async fn test_check_free_tier_reached() {
        let test_cases = get_test_cases().await;
        for test_case in test_cases {
            let free_limit_reached = check_free_tier_reached(
                &test_case.rate_limit_db,
                &test_case.user_address,
                test_case.free_tier_rate_limit_requests_per_second,
            )
            .await;
            assert_eq!(free_limit_reached, test_case.expected_output);

            // Check that the timestamps was updated.
            let usage_map = test_case
                .rate_limit_db
                .free_tier_wallet_usage_map
                .read()
                .await;
            let timestamps = usage_map
                .get(&test_case.user_address)
                .unwrap()
                .timestamps
                .read()
                .await;
    ...
    async fn get_test_cases() -> Vec<TestCase> {
        ...
        vec![
            ...
            TestCase {
                rate_limit_db: RateLimitDB::default_with_chain_data_config_manager(cdm.clone()),
                user_address: "123".into(),
                free_tier_rate_limit_requests_per_second: 0.2,
                expected_output: false,
                expected_timestamps_length: 1,
            },
            // Wallet entry exists with expired timestamps.
            TestCase {
                rate_limit_db: {
                    let rate_limit_db =
                        RateLimitDB::default_with_chain_data_config_manager(cdm.clone());
            ...
            // Wallet entry exists with timestamps below threshold.
            TestCase {
                rate_limit_db: {
                    let rate_limit_db =
                        RateLimitDB::default_with_chain_data_config_manager(cdm.clone());
                ...
                user_address: "123".into(),
                free_tier_rate_limit_requests_per_second: 0.034,
                expected_output: false,
                expected_timestamps_length: 2,
            },
            // Wallet entry exists with timestamps at threshold.
            TestCase {
                rate_limit_db: {
                    let rate_limit_db =
                        RateLimitDB::default_with_chain_data_config_manager(cdm.clone());
                ...
                user_address: "123".into(),
                free_tier_rate_limit_requests_per_second: 1.0 / 30.0,
