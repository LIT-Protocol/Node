...
pub async fn node_fsm_worker(
    ...
    if let Ok(true) = cfg.load().enter_restore_state() {
        trace!("enter_restore_state is set in the config. Attempting to restore.");

        let mut recovered_bls_pub_keys = vec![];
        let mut recovered_ecdsa_pub_keys = vec![];
        // Try to restore the key shares until all the key shares are restored.
        loop {
            // Try to restore the key shares under the read lock.
            let (mut newly_recovered_bls_keys, mut newly_recovered_ecdsa_keys) = {
                let state = restore_state.read().await;
                let epoch = peer_state.epoch().await;
                state.try_restore_key_shares(epoch).await
            };
            ...
            // Save the pub keys that are available.
            recovered_bls_pub_keys.append(&mut newly_recovered_bls_keys);
            recovered_ecdsa_pub_keys.append(&mut newly_recovered_ecdsa_keys);

            // Check if the restoration is complete for this node.
            let restoration_complete_node = {
                let state = restore_state.read().await;
                state.are_all_keys_restored()
            };

            // Check if the restoration is complete for the network.
            let restoration_complete_network = match peer_state.network_state().await {
                Ok(NetworkState::Restore) | Ok(NetworkState::Paused) => false,
                Ok(_) => true,
                Err(_) => false,
            };

            // If restoration is complete either for the node or for the network,
            // exit the loop.
            if restoration_complete_node || restoration_complete_network {
                break;
            }
...
async fn perform_epoch_change(
    ...
    async fn _perform_epoch_change(
        ...
        for epoch_manager in epoch_managers {
            let key_type = epoch_manager.key_type();
            let epoch_dkg_id = format!("{}.{}.{}", dkg_id, key_type, dkg_type);
            match epoch_manager
                .change_epoch(
                    epoch_dkg_id,
                    epoch_number.as_u64(),
    ...
    while abort_and_restart_count < 10 {
        ...
        let epoch_change_res_or_update_needed = tokio::select! {
            ...
            res = _perform_epoch_change(cfg, epoch_managers, latest_dkg_id.clone(), epoch_number, &current_peers, &new_peers, dkg_type) => {
                EpochChangeResOrUpdateNeeded {
...
pub async fn get_fsm_state(
    ...
    let peers_in_epoch = peer_state.peer_socket_addresses().await?;
...
pub async fn get_current_and_new_peer_addresses(
    peer_state: Arc<PeerState>,
) -> Result<(Vec<PeerSocketAddress>, Vec<PeerSocketAddress>)> {
    peer_state.connect_to_validators_union().await?;

    let current_peers = peer_state.peer_socket_addresses().await?;

    let new_peers = peer_state.peer_socket_addresses_in_next_epoch().await?;

    info!(
        "Current/new peers for epoch change: ( {}/{} )  {:?} / {:?} ",
        &current_peers.len(),
        &new_peers.len(),
        &current_peers.active_addresses(),
        &new_peers.all_addresses()
    );
...
pub async fn vote_for_root_pubkeys(
    ...
    for dkg_root_key in pubkeys {
        ...
        let rootkey = RootKey {
            ...
            key_type: dkg_root_key.key_type.into(),
        };
...
#[cfg(test)]
mod test {
    use crate::tasks::fsm::parse_epoch_number_from_dkg_id;
