...
pub async fn real_time_metrics_worker(
                ...
                match msg {
                    ...
                    MetricsMessage::Poll(tx) => {
                        let poll = MetricPoll {
                            ...
                            peers: validators.iter().map(|v| SimplePeer { idx: v.share_index as u16, kicked: v.kicked }).collect(),
                            time: SystemTime::now()
                        };
                        ...
                        validators = match peer_state.peer_socket_addresses().await {
                        ...
                        validators.iter().for_each(|v| metrics.push(RealTimeCounter { idx: v.share_index as u16,   inmsg: 0, outmsg: 0, badmsg: 0, time: SystemTime::now() }));
    ...
    async fn init_counters(
        ...
        let validators = match peer_state.peer_socket_addresses().await {
            Ok(validators) => validators,
            Err(_) => {
                error!("rt-metrics: peer_state.validators_in_current_epoch().await is_err()");
                Vec::new()
