...
pub(super) fn extract_and_verify(siwe_message: Message) -> Result<impl SessionCapabilityObject> {
    let capability_object =
        Capability::<Value>::extract_and_verify(&siwe_message).map_err(|err| {
            parser_err_code(
                err,
                EC::NodeSIWECapabilityInvalid,
                Some("Unable to extract and verify capability object in SIWE message".into()),
            )
            .add_msg_to_details()
        })?;

    Ok(RecapSessionCapabilityObject {
        inner: capability_object.ok_or_else(|| {
            validation_err_code(
                "SIWE missing capability object".to_string(),
                EC::NodeSIWECapabilityInvalid,
                None,
            )
        })?,
...
#[derive(Debug)]
pub(super) struct RecapSessionCapabilityObject {
...
mod extract_and_verify_tests {
    ...
    use crate::{auth::resources::AccessControlConditionResource, error::EC};

    use super::extract_and_verify;
    use crate::auth::capabilities::SessionCapabilityObject;

    #[test]
    fn test_extract_and_verify_with_caps() {
        let siwe_message = Message {
            domain: "example.com".parse().unwrap(),
            address: Default::default(),
            statement: Some(r#"Some custom statement. I further authorize the stated URI to perform the following actions on my behalf: (1) 'Threshold': 'Decryption' for 'lit-accesscontrolcondition://*'."#.into()),
            ...
            not_before: None,
            request_id: None,
            resources: vec!["urn:recap:eyJhdHQiOnsibGl0LWFjY2Vzc2NvbnRyb2xjb25kaXRpb246Ly8qIjp7IlRocmVzaG9sZC9EZWNyeXB0aW9uIjpbe31dfX0sInByZiI6W119Cg==".parse().unwrap()],
        };

        let session_capability_object = extract_and_verify(siwe_message);
        assert!(session_capability_object.is_ok());
        let session_capability_object = session_capability_object.unwrap();
    ...
    fn test_extract_and_verify_no_caps() {
        ...
        let session_capability_object = extract_and_verify(siwe_message);
        assert!(session_capability_object.is_err());

        let err = session_capability_object.unwrap_err();
