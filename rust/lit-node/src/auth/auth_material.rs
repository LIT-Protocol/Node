...
impl AuthSigItem {
    pub(crate) async fn validate_and_get_user_address(
        ...
        let user_address = valid_wallet_sig.user_address().await.map_err(|e| {
...
impl JsonAuthSig {
    ...
    #[async_recursion]
    pub async fn is_evm_user_address(&self) -> Result<bool> {
            ...
            || matches!(self.auth_material_type, AuthMaterialType::ContractSig)
        {
            ...
            let inner_wallet_sig = extract_wallet_sig(self).await?;
            return inner_wallet_sig.is_evm_user_address().await;
        }
    ...
    pub async fn validate_and_get_wallet_sig(
        ...
        let mut new_auth_sig = match self.auth_material_type {
            AuthMaterialType::WalletSig => {
                validate_wallet_sig_by_chain(self, chain).await?;
                self.clone()
            }
            ...
            AuthMaterialType::SessionSig => {
                ...
                #[allow(deprecated)]
                if let Ok(valid_auth_sig) =
                    validate_session_sig(self, requested_lit_resource_ability, chain, cfg).await
                {
                    valid_auth_sig
                } else {
...
impl<'de> Visitor<'de> for JsonAuthSigVisitor {
        ...
        M: MapAccess<'de>,
    {
        ...
        let auth_material_type = JsonAuthSig::determine_auth_material_type(
            &sig,
            &derived_via,
            &signed_message,
            &address,
