...
pub(crate) async fn validate_wallet_sig_by_chain(
    ...
    match chain {
        Some(chain) => match chain.as_str() {
            ...
            _ => validate_wallet_sig(auth_sig).await,
        },
        None => validate_wallet_sig(auth_sig).await,
    }
}

/// Validate EVM-compatible wallet signature.
async fn validate_wallet_sig(auth_sig: &JsonAuthSig) -> Result<()> {
...
mod tests {
    ...
    async fn test_validate_invalid_sig() {
        ...
        let result = validate_wallet_sig(&auth_sig).await;
