import { Uint8arrays } from "ext:lit/01_uint8arrays.js";
...
function pubkeyToTokenId({ publicKey }) {
  return Deno[Deno.internal].core.ops.op_pubkey_to_token_id(publicKey);
}
...
function setResponse({ response }) {
  return Deno[Deno.internal].core.ops.op_set_response(response);
}
 ...
 * @param {Object} params.params The parameters to pass to the child Lit Action
...
function call({ ipfsId, params }) {
  return Deno[Deno.internal].core.opAsync("op_call_child", ipfsId, params);
}
...
function callContract({ chain, txn }) {
  return Deno[Deno.internal].core.opAsync("op_call_contract", chain, txn);
}
...
function claimKey({ keyId }) {
  return Deno[Deno.internal].core.opAsync("op_claim_key_identifier", keyId);
}
...
globalThis.LitActions = LitActions;
