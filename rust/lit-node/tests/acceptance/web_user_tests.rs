...
use common::node_collection::NodeCollection;
...
pub async fn test_encryption_decryption_auth_sig(nc: &NodeCollection, scenario: &Scenario) {
    // prepare
    let (
        to_encrypt,
        data_to_encrypt_hash,
    ...
    // Encrypt.
    let network_pubkey = get_network_pubkey(scenario).await;
    ...
    // Assert decryption
    assert_decrypted(
        scenario.actions.get_current_validator_count().await,
        &network_pubkey,
        identity_param,
        &to_encrypt,
        &ciphertext,
...
async fn test_encryption_decryption_session_sigs(nc: &NodeCollection, scenario: &Scenario) {
    // prepare
    let (
        to_encrypt,
        data_to_encrypt_hash,
...
async fn retrieve_decryption_key(
    ...
    let get_encryption_key_resp =
        hit_endpoints_with_json_body(scenario, "web/encryption/sign".into(), payload.to_string())
            .await;
...
async fn retrieve_decryption_key_session_sigs(
    nc: &NodeCollection,
    scenario: &Scenario,
    access_control_conditions: Option<Vec<AccessControlConditionItem>>,
    evm_contract_conditions: Option<Vec<EVMContractConditionItem>>,
...
#[cfg(feature = "lit_actions")]
mod lit_actions_extesions {
    #[allow(dead_code)]
    async fn test_lit_action_auth_sig(nc: &NodeCollection, s: &Scenario) {
        let action_result = common::lit_actions::sign_hello_world(nc, s).await;
        assert!(action_result.is_ok());
    }

    /// TODO: This test doesn't actually test session sigs correctly, we still need to integrate
    /// full verification logic into the node.
    #[allow(dead_code)]
    async fn test_lit_action_session_sigs(nc: &NodeCollection, s: &Scenario) {
        // Get session sig for auth
        let session_sigs = get_session_sigs_for_auth(
            vec![LitResourceAbilityRequest {
                resource: LitResourceAbilityRequestResource {
                    resource: "1337".into(),
                    resource_prefix: "lit-la".to_string(),
                },
        ...
        // run
        let action_result =
            common::lit_actions::sign_hello_world_session_sigs(nc, s, &session_sigs).await;
        assert!(action_result.is_ok());
    }
}
...
async fn test_everything_as_web_user() {
    ...
    let (nc, scenario) = new_node_collection(num_staked, num_nodes, None, true, false).await;
    ...
    test_encryption_decryption_auth_sig(&nc, &scenario).await;

    info!("Testing decryption with session sigs");
    test_encryption_decryption_session_sigs(&nc, &scenario).await;

    // FIXME: Get this test working.
    // {
    //     info!("Testing lit actions with auth sigs");
    //     test_lit_action_auth_sig(&nc, &_s).await;

    //     info!("Testing lit actions with session sigs");
    //     test_lit_action_session_sigs(&nc, &_s).await;
    // }
...
async fn test_rate_limit_nft() {
    ...
    let s = Scenario::builder()
        ...
        .build()
        .await;
    let (node_collection, scenario) = NodeCollection::from_scenario(s).await;
    ...
    // Get auth sig for auth
    let wallet = LocalWallet::new(&mut OsRng).with_chain_id(scenario.testnet.chain_id);
    let auth_sig = generate_authsig(&wallet)
        .await
        .expect("Couldn't generate auth sig");
    ...
    let free_requests_per_rate_limit_window = scenario
        .contracts
        .rate_limit_nft
    ...
    let provider = scenario.testnet.provider.clone();
    fund_wallet(&provider, &wallet, "100000000000000000000").await;

    // mint a rate limit increase nft
    let rate_limit_nft_contract_address = scenario.contracts.rate_limit_nft.address();
    ...
    let auth_sig_for_session_sig_that_should_fail =
        get_auth_sig_for_session_sig("meow".to_string(), Some(wallet.clone()));
    // Retrieve decrypted key
    let decryption_resp = retrieve_decryption_key(
        &node_collection,
        &scenario,
    ...
    let rate_limit_window_secs = scenario
        .contracts
        .rate_limit_nft
    ...
    let rate_limit_holder_wallet =
        LocalWallet::new(&mut OsRng).with_chain_id(scenario.testnet.chain_id);
    ...
    // // 3. try to use the delegation auth sig as a regular auth sig and show that it doesn't work without the proper resources
    // info!("Testing that the delegation auth sig doesn't work as a regular auth sig without the proper resources");
    // // send requests until we hit the rate limit
    // let mut count_after = 0;
    // loop {
    //     info!(
    ...
    // }
    // info!(
    //     "We made {} requests after minting the NFT and trying to use the delegation signature as an AuthSig before we hit the rate limit again",
    //     count_after
    // );

    // 4. try to use the delegation auth sig with the proper resources, to show that a
    // developer can just send an auth sig directly to pay for their user requests,
    // so that they don't need to get a session sig from a PKP which uses up a request
}
