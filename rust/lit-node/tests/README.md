...
Integration tests typically involve a testnet (representing the underlying chain), a scenario (the base configuration of contracts and quantity of nodes involved) and a quantity of nodes.   Running a test often has configurable parameters - ie, *mint a PKP on `mumbai` using a 6 node network and sign a lit action*.  
