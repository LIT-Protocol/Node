...
pub fn generate_custom_node_runtime_config(
    ...
    let advance_epoch = which_testnet != WhichTestnet::NoChain;
    ...
    #[cfg(not(all(feature = "proxy_http", feature = "testing")))]
    {
        cfg.insertstr(
            section,
            CFG_KEY_ECDSA_ROUND_TIMEOUT,
            &custom_config.ecdsa_round_timeout.unwrap_or("20000".into()),
        );
    }

    if let Some(chain_polling_interval) = custom_config.chain_polling_interval {
        cfg.insertstr(
            section,
    ...
    cfg.insertstr(
        section,
        CFG_KEY_ENABLE_RATE_LIMITING,
        &custom_config.enable_rate_limiting.unwrap_or("false".into()),
    );
