...
use crate::common::models::VotingStatusToKickValidator;
use crate::common::node_collection::{handshake_returns_keys, NodeStakingStatus};
use anyhow::Result;
...
impl Actions {
    ...
    pub async fn sendapprovestake(
        ...
        use lit_blockchain::contracts::staking::StakingErrors;
        ...
        info!(
            "Staking from {:?} for with node_address {:?}",
            staker.address(),
            node_info.node_address
        );

        let r = staking.stake_and_join(
            stake_amount,
            ip,
            1, //check
            port,
        ...
        if let Err(e) = r {
            debug!(
                "Error doing stake_and_join for {:}.  Revert: {:?}",
                node_info.addr,
            ...
            match revert {
                Some(r) => {
                    return Err(anyhow::anyhow!(
                        "Error doing stake_and_join {:} : {:?}.  Revert: {:?}",
                        node_info.addr,
                        e,
                        r
                    ));
                ...
                None => {
                    return Err(anyhow::anyhow!(
                        "Error doing stake_and_join {:} : {:?}.  Could not decode revert reason.  Revert: {:?}",
    ...
    pub async fn wait_for_root_keys(&self, scenario: &Scenario) -> bool {
        info!("Waiting for root keys!");

        // First, check whether the root keys are registered on the chain.
        loop {
            if self.get_root_keys(KeyType::EcdsaCaitSith).await.is_some()
                && self.get_root_keys(KeyType::BLS).await.is_some()
            {
                break;
            }
            let _r = tokio::time::sleep(std::time::Duration::from_secs(1)).await;
        ...
        loop {
            if handshake_returns_keys(scenario).await {
                break;
            }
            let _r = tokio::time::sleep(std::time::Duration::from_secs(1)).await;
        }
    ...
    pub async fn get_root_keys(&self, key_type: KeyType) -> Option<Vec<String>> {
        ...
        let root_keys: Vec<String> = all_root_keys
            .iter()
            .filter(|k| k.key_type == key_type.into())
    ...
    pub async fn register_backup_parties(&self, party_members: Vec<Address>, owner: &NodeAccount) {
        let backup_recovery =
        ...
        let tx = backup_recovery.register_new_backup_party(party_members);
    ...
    pub async fn increase_blockchain_timestamp(&self, seconds_to_increase: usize) {
        ...
        let block = self
            .provider
            .get_block(self.provider.get_block_number().await.unwrap())
            .await
            .unwrap()
    ...
    pub async fn fast_forward_blocks(&self, blocks_to_mine: usize) {
        ...
        let block_num_before = self.provider.get_block_number().await.unwrap();
        ...
        let block_num_after = self.provider.get_block_number().await.unwrap();
        info!(
    ...
    pub async fn wait_for_epoch(&self, epoch: U256) {
        info!(
            "Waiting for next epoch.  Current epoch is {}.",
    ...
    // fix_me :: this should only be done if we want to start in a staked mode and we aren't testing out staking.
    pub async fn ensure_node_staked(&self, node_account: NodeAccount) -> Result<NodeStakingStatus> {
        ...
        // get the comms keys from the contract.  this info was already set in the past from the JS deploy script.  we just haven't staked the node yet.
        let validator_struct = self
            .contracts
            .staking
            .validators(node_account.staker_address)
            .call()
            .await
            .unwrap();
        ...
        let peer_item = PeerItem::new(
            ...
            validator_struct.sender_pub_key.into(),
            validator_struct.receiver_pub_key.into(),
        ...
        if is_staked.is_ok() && is_staked.unwrap() {
            info!("Node {} is already staked!", node_signer.address());
            return Ok(NodeStakingStatus::PreviouslyStaked);
        }

        info!("Node {} is not staked.  Staking...", node_signer.address());
        self.sendapprovestake(
            node_signer,
            2130706433u32,
            node_account.port as u32,
            &peer_item,
        )
        .await?;

        Ok(NodeStakingStatus::Staked)
    }
}
