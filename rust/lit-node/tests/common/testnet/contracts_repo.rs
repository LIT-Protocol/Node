...
use crate::common::testnet::contracts::ContractAddresses;

use super::{NodeAccount, SimpleTomlValue, Testnet};
...
impl WalletManifestItem {
    pub fn map_to_node_account(&self, provider: Provider<Http>, chain_id: u64) -> NodeAccount {
        let addr = format!("127.0.0.1:{}", 7470 + self.idx);
    ...
    pub fn assert_against_node_config(&self) {
        let config_file = &format!(
            "../../blockchain/contracts/node_configs/lit_config{}.toml",
            self.idx
        );
        ...
        let node_sk_bytes = SigningKey::from_bytes(GenericArray::from_slice(&node_private_key))
            .expect("Couldn't parse the received key")
            .to_bytes();
        let thing = SigningKey::from_bytes(GenericArray::from_slice(
            &config
                .get_signing_key()
...
pub async fn remote_deployment_and_config_creation(num_staked: usize, num_nodes: usize) -> bool {
    ...
    {
        deploy_node_config["numberOfNodeWallets"] = num_nodes.into();
        deploy_node_config["numberOfStakedWallets"] = num_staked.into();

        // Set the IP addresses
        let ip_addresses = default_staker_ip_addresses(7470, num_nodes);
...
pub async fn contract_addresses_from_deployment(_testnet: &Testnet) -> ContractAddresses {
    // extract the addresses from the deployment script output
    let deployed_core_contracts_path =
        "../../blockchain/contracts/deployed-lit-core-contracts-temp.json";
    let deployed_node_contracts_path =
        "../../blockchain/contracts/deployed-lit-node-contracts-temp.json";
...
pub async fn save_to_chain_state_cache(
    ...
    if latest_wallet_manifest.len() != num_nodes {
