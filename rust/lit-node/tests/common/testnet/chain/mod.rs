...
use crate::common::testnet::contracts_repo::latest_wallet_manifest;
...
pub trait ChainTrait {
    ...
    fn rpc_url(&self) -> String {
        ...
        lit_blockchain::resolver::rpc::rpc_url(chain_id, 0).expect("invalid chain id")
    }
    ...
    fn accounts(&self) -> Arc<Vec<NodeAccount>> {
        ...
        if latest_wallet_manifest.len() != self.num_nodes() {
            panic!("Number of nodes in latest_wallet_manifest.json does not match num_nodes in chain config");
        }
        ...
        Arc::new(
            latest_wallet_manifest
                ...
                .map(|wallet_manifest_item| {
                    let mut provider =
                        get_provider(self.chain_name(), 0).expect("Error getting provider");
    ...
    fn deployer(&self) -> NodeAccount {
        ...
        let provider = lit_blockchain::resolver::rpc::get_provider(self.chain_name(), 0).unwrap();
