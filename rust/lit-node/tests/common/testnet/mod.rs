...
use crate::common::testnet::contracts_repo::remote_deployment_and_config_creation;

use self::chain::ChainTrait;
use self::contracts::Contracts;
    ...
    pub async fn build(self) -> Testnet {
        let chain = match self.which {
            WhichTestnet::Hardhat => {
                Box::new(chain::hardhat::Hardhat::new(self.num_nodes)) as Box<dyn ChainTrait>
            }
            WhichTestnet::Anvil => {
                Box::new(chain::anvil::Anvil::new(self.num_nodes)) as Box<dyn ChainTrait>
            }
            WhichTestnet::NoChain => {
                Box::new(chain::no_chain::NoChain::new(self.num_nodes)) as Box<dyn ChainTrait>
            }
        };

        let net_process = chain.start_chain().await;
        let mut provider_mut = get_provider(chain.chain_name(), 0)
        ...
        if self.which != WhichTestnet::NoChain {
                ...
                && !self.force_deploy_in_ci
            {
                ...
                .await
                {
                    remote_deployment_and_config_creation(self.num_staked, self.num_nodes).await;
