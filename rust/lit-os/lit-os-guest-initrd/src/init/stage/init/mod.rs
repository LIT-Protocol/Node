...
use cryptsetup_rs::Luks2CryptDevice;
...
pub(crate) async fn run(_ctx: &mut InitContext) -> Result<Outcome> {
    // I couldn't find it on brew anyway.
    unimplemented!("cryptsetup not supported on aarch64 (apple silicon / M1)")
}
...
async fn init_luks_volumes(
    ctx: &mut InitContext, root_dev: &mut impl Luks2CryptDevice, root_dev_label: &String,
    root_passphrase: &[u8], var_dev: &mut impl Luks2CryptDevice, var_dev_label: &String,
...
async fn init_luks_volume(
    ctx: &mut InitContext, dev: &mut impl Luks2CryptDevice, label: &String, init_passphrase: &[u8],
    boot_passphrase: &[u8],
) -> Result<()> {
    ...
    // Reload device
    let mut dev = cryptsetup_init_dev(dev.path(), label)?;
