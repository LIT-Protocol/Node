...
use crate::utils::cryptsetup_sys::{cryptsetup_activate_dev, cryptsetup_deactivate_dev};
...
pub(crate) fn deactivate_luks_root_volume(ctx: &mut InitContext) -> Result<()> {
    let (_root_dev, root_dev_label) = root_volume_dev_and_label(ctx)?;

    info!("Deactivating volume '{}'", root_dev_label);
    cryptsetup_deactivate_dev(root_dev_label.as_str(), LUKS_NAME_ROOT)?;

    Ok(())
}

pub(crate) fn deactivate_luks_var_volume(ctx: &mut InitContext) -> Result<()> {
    let (_var_dev, var_dev_label) = var_volume_dev_and_label(ctx)?;

    info!("Deactivating volume '{}'", var_dev_label);
    cryptsetup_deactivate_dev(var_dev_label.as_str(), LUKS_NAME_VAR)?;
