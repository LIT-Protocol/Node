...
// Replaced using Luks2CryptDevice so we can use the read only option.
pub fn cryptsetup_activate_dev(
    dev_path: &Path, label: &str, name: &str, passphrase: &[u8], read_only: bool,
) -> Result<()> {
    ...
    let mut child = cmd.stderr(Stdio::inherit()).stdout(Stdio::inherit()).spawn().map_err(|e| {
        generic_err(e, Some(format!("failed to 'cryptsetup open' '{label}' luks device")))
    })?;

    let res = child.wait().map_err(|e| {
        generic_err(
            e,
            Some(format!("failed while waiting for 'cryptsetup open' '{label}' luks device")),
        )
    })?;
    ...
    if !res.success() {
        return Err(generic_err(
            format!(
                "failed to exec 'cryptsetup open' '{}' luks device: status: {:?}",
                label,
                res.code()
            ),
            None,
...
pub fn cryptsetup_deactivate_dev(label: &str, name: &str) -> Result<()> {
    let mut cmd = Command::new("cryptsetup");
    cmd.arg("close").arg(name).arg("-q");

    let mut child = cmd.stderr(Stdio::inherit()).stdout(Stdio::inherit()).spawn().map_err(|e| {
        generic_err(e, Some(format!("failed to 'cryptsetup close' '{label}' luks device")))
    })?;

    let res = child.wait().map_err(|e| {
        generic_err(
            e,
            Some(format!("failed while waiting for 'cryptsetup close' '{label}' luks device")),
        )
    })?;
