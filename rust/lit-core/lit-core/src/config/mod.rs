...
impl LitConfig {
    ...
    // Local config
    pub fn save_local_config(&self, data: &HashMap<String, String>) -> Result<()> {
        ...
        self.save_toml_as_local_config(&config_toml)?;

        Ok(())
    }

    // Local config
    pub fn save_toml_as_local_config(&self, toml: &SimpleToml) -> Result<()> {
        let config_file = PathBuf::from(format!("./{CFG_NAME}.{CFG_EXT}"));
