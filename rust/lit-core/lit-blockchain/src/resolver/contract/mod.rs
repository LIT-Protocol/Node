...
use crate::contracts::{
    ...
    MULTI_SENDER_CONTRACT, PKP_HELPER_CONTRACT, PKP_NFT_CONTRACT, PKP_NFT_METADATA_CONTRACT,
    PKP_PERMISSIONS_CONTRACT, PUB_KEY_ROUTER_CONTRACT, RATE_LIMIT_NFT_CONTRACT,
    RELEASE_REGISTER_CONTRACT, STAKING_BALANCES_CONTRACT, STAKING_CONTRACT,
};
...
impl ContractResolver {
        ...
        K: AsRef<str> + Into<String> + Send,
    {
        ...
        // Resolve
        let address = self.resolve_address(cfg, &contract_key).await?;
    ...
    pub async fn backup_recovery_contract(
        ...
        BackupRecovery::load(cfg, *self.resolve(cfg, "BACKUP_RECOVERY").await?.address())
    }
