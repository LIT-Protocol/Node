...
import { mapEnvToEnum, verifyContractInBg } from "./utils";
...
export async function deployLitCoreContracts(
    ...
    const contractResolver = await deployContract(
        chainName,
        "ContractResolver",
    ...
    const releaseRegister = await deployContract(chainName, "ReleaseRegister", [
        mapEnvToEnum(deployCoreConfig.environment),
    ]);
