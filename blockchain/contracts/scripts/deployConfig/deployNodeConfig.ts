...
export async function askDeployNodeConfig(
    ...
    const numberOfNodeWallets = await askForNumberOfNodeWallets();

    const keyTypes = await askForKeyTypes();

    const ipAddresses = (await askForIpAddresses(numberOfNodeWallets)).split(
        ","
    );
    ...
    const deployConfig: DeployNodeConfig = {
        ...
        numberOfNodeWallets,
        numberOfStakedWallets: numberOfNodeWallets,
    ...
    const existingAddresses = await inquirer.prompt([
        {
            ...
            message: `Enter the JSON for the existing DomainOracle and DomainWalletRegistry contracts`,
            validate: (input) => {
                try {
                    const a = JSON.parse(input);
                    if (a) {
...
// Ask the user how many wallets they would like to create and fund.
async function askForNumberOfNodeWallets(): Promise<number> {
    const numberOfNodeWallets = await inquirer.prompt([
        {
            type: "number",
            name: "numberOfNodeWallets",
            message: `How many node wallets would you like to create & fund?`,
            default: 10,
            validate: (input) => input >= 0,
        },
    ]);

    return numberOfNodeWallets.numberOfNodeWallets;
}

async function askForIpAddresses(numberOfNodeWallets: number): Promise<string> {
    return askForEnvConfirmOrInput(
        "IP_ADDRESSES",
        ...
        (input) => {
            try {
                const ipAddresses = input
                    .split(",")
                    .filter((a) => a.length > 0);
                if (ipAddresses.length !== numberOfNodeWallets) {
                    return `You entered ${ipAddresses.length} IP addresses, but you said you wanted to create ${numberOfNodeWallets} wallets.`;
                }
            } catch (e) {
                return `Error parsing input: ${e}`;
            }
        ...
        generateDefaultIpAddresses(numberOfNodeWallets)
    );
}
    ...
    const keys = await inquirer.prompt([
        {
            ...
            message:
                "Enter encryption (backup party pub) keys, seperated by comma",
            default: "none",
            validate: (input) => {
                if (input === "none") {
                    return true;
...
function generateDefaultIpAddresses(numberOfNodeWallets: number): string {
    let ipAddresses = "";
    for (let i = 0; i < numberOfNodeWallets; i++) {
