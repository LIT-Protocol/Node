...
async function joinNetwork(
    ...
    console.info("Requesting to join network", {
        aliasBalanceBefore: ethers.formatEther(
            await ethers.provider.getBalance(walletToJoinWith.getAddress())
        ),
        aliasIp: toBigInt(ip2int(validatorConfig.ip)),
        aliasPort: toBigInt(validatorConfig.port),
        aliasNodeAddress: validatorConfig.nodeAddress,
        aliasCommsSenderPubkey: validatorConfig.commsSenderPubkey,
        aliasCommsReceiverPubkey: validatorConfig.commsReceiverPubkey,
    });
