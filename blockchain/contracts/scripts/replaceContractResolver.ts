...
async function run() {
    ...
    const newContractResolver: ContractResolver = await deployContract(
        hre.network.name,
        "ContractResolver",
        [ENV]
    );
    ...
    const allTyps = await Promise.all([
        ...
        oldContractResolver.DOMAIN_WALLET_ORACLE(),
    ...
    updateTx = await pubkeyRouter.setContractResolver(
        newContractResolverAddress
    );
    console.log("updateTx: ", updateTx.hash);
    await updateTx.wait();
