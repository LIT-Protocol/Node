...
const stakeTokensAndLockValidatorSet = async (
    ...
    for (let i = 0; i < nodeOperatorsCredentials.length; i++) {
        ...
        const connectedStakerWallet =
            nodeOperatorCredential.stakerWallet.connect(ethers.provider);
...
export async function fundAndStakeNodes(
    ...
    if (deployNodeConfig.numberOfNodeWallets === 0) {
    ...
    let nodeOperatorsCredentials = generateWallets(
        ethers,
        deployNodeConfig.numberOfNodeWallets
    );
    ...
    await stakeTokensAndLockValidatorSet(
        nodeOperatorsCredentials.slice(
            0,
            deployNodeConfig.numberOfStakedWallets
        ),
        contracts
    );

    if (deployNodeConfig.ipAddresses) {
        // *** 6. Set the IP addresses in the staking contract
