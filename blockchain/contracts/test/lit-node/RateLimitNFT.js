    ...
    describe("Test minting past max capacity", async () => {
        ...
        it("tries to mint past the max capacity with many NFTs", async () => {
            ...
            for (let i = 0; i < toMint; i++) {
                ...
                const txn = await rateLimitNFTContract.mint(
                    expirationTimeForThisNft,
                    {
                        value: cost,
                    }
                ...
                expect(totalSoldCapacity).to.equal(
                ...
                expect(totalSoldRequestsPerKilosecondByExpirationTime).to.equal(
                    10n
                );
                // console.log(
            ...
            let cost = await rateLimitNFTViewsContract.calculateCost(
                11n,
                expiresAt
            );
