...
describe("Staking", function () {
    ...
    describe("validator joining", function () {
        ...
        it("can join as a validator, and cannot leave as a staker if below min validator count", async () => {
            ...
            let initialTokenBalance = await token.balanceOf(
                stakingAccount1.address
            );
            ...
            let postTokenBalance = await token.balanceOf(
                stakingAccount1.address
            );
            ...
            initialTokenBalance = await token.balanceOf(
                stakingAccount1.address
            );
            ...
            postStakeBal = await stakingBalances.balanceOf(
                stakingAccount1.address
            );
            ...
            postBalance = await stakingBalances.balanceOf(
                stakingAccount1.address
            );
        ...
        it("kicks and slashes validator", async () => {
            ...
            // Lock, signal and advance epoch.
            await lockSignalAndAdvanceEpoch(
                stakingContract,
                stakingViewsFacet,
                {
                    stakingAccounts: stakingAccounts.slice(
                        0,
                        stakingAccounts.length - 1
                    ),
                }
            );
        ...
        it("cannot kick such that less than a threshold of validators from current set remain", async () => {
            ...
            const numSuccessfulKicks =
                stakingAccountsCopy.length - thresholdCount;
            ...
            for (let i = 0; i < numSuccessfulKicks; i++) {
                ...
                const stakingAccountToBeKicked =
                    stakingAccountsCopy[indexToKickFrom];
            ...
            // Now, try signalling ready for next epoch and advancing epoch
            await lockSignalAndAdvanceEpoch(
                stakingContract,
                stakingViewsFacet,
                {
    ...
    describe("Version tests", () => {
        it("Can get min and max version", async () => {
            let minVersionString =
                await stakingVersionFacet.getMinVersionString();
            expect(minVersionString).to.equal("0.0.0");

            let maxVersionString =
                await stakingVersionFacet.getMaxVersionString();
    ...
    describe("when paused", () => {
        ...
        afterEach(async () => {
            await stakingContract
                .connect(deployer)
                .setEpochState(stateBeforePause);
        });
...
async function updateMinimumValidatorCount(
    ...
    await stakingContract.setConfig(
        currentConfig.tokenRewardPerTokenPerEpoch,
        currentConfig.complaintTolerance,
        currentConfig.complaintIntervalSecs,
        [...(await stakingViewsFacet.getKeyTypes())],
        newMinimumValidatorCount,
        currentConfig.maxConcurrentRequests,
        currentConfig.maxTripleCount,
        currentConfig.minTripleCount,
        currentConfig.peerCheckingIntervalSecs,
        currentConfig.maxTripleConcurrency
    );
}
...
async function makeAssertions(
    ...
    if (!!expectedValidatorsInNextEpoch) {
        expect(
            (await stakingViewsFacet.getValidatorsInNextEpoch()).length
    ...
    if (expectedValidatorsInCurrentEpochExcludes.length > 0) {
        for (const excludedValidatorAddress of expectedValidatorsInCurrentEpochExcludes) {
            expect(
                (
                    await stakingViewsFacet.getValidatorsInCurrentEpoch()
                ).includes(excludedValidatorAddress)
            ).is.false;
        }
    }
