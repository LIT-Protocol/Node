...
describe("BackupRecovery", function () {
    ...
    it("should register recovery keys", async () => {
        ...
        const blsKey = {
            pubkey: "0x028506cbedca1d12788d6bc74627d99263c93204d2e9565d861b7c1270736b0071",
            keyType: 1,
        };

        // ECDSA pubKey should be different from BLS pubKey
        const ecdsaKey = {
            pubkey: "0x018506cbedca1d12788d6bc74627d99263c93204d2e9565d861b7c1270736b0071",
            keyType: 2,
        };
        ...
        for (let i = 0; i < backupPartyCount - 1; i++) {
            ...
            expect(await backupRecoveryContract.isRecoveryDkgCompleted()).to.be
                .false;
        }
    ...
    it("should map validators to backup until equals backup count", async () => {
        ...
        for (let i = 0; i < stakingAccounts.length; i++) {
            ...
            if (i < backupPartyCount) {
                ...
                let peerAddresses =
                    await backupContract.getNodeAddressesForDkg();
    ...
    it("should submit valid proof", async () => {
        ...
        let proof = new Uint8Array([
            1, 0, 0, 0, 0, 101, 92, 43, 178, 0, 157, 137, 108, 202, 42, 239,
            133, 106, 124, 17, 78, 140, 254, 165, 166, 3, 68, 236, 72, 237, 26,
            60, 125, 231, 225, 12, 198, 231, 69, 129, 98, 109, 10, 125, 19, 128,
            146, 177, 152, 192, 20, 234, 151, 23, 232, 132, 192, 3, 16, 94, 72,
            223, 175, 141, 9, 136, 150, 119, 236, 165, 211, 136, 243, 6, 175,
            213, 176, 39, 182, 105, 20, 182, 3, 76, 186, 159, 25, 55, 132, 193,
            4, 131, 33, 255, 109, 25, 248, 87, 34, 197, 244, 124, 144, 117, 142,
            200, 243, 140, 168, 103, 244, 154, 71, 158, 211, 131, 180, 42, 189,
            242, 137, 170, 2, 61, 106, 241, 24, 60, 97, 169, 160, 126, 36, 139,
            117, 207, 195, 70, 18, 148, 72, 60, 5, 98, 15, 242, 4, 228, 55, 81,
            61, 187, 184, 79, 250, 202, 214, 148, 29, 54, 183, 128, 31, 56, 98,
            216, 97, 144, 112, 206, 7, 62, 245, 2, 197, 51, 240, 12, 2, 139, 72,
            208, 82, 192, 50, 72, 237, 47, 90, 92, 197, 233, 31, 36, 161, 76,
            144, 79, 52, 57, 215, 43, 204, 175, 236, 205, 109, 130, 15, 40, 158,
            218, 244, 129, 136, 4, 126, 85, 15, 34, 7, 188, 110, 29, 83, 56, 69,
            229, 9, 136, 65, 119, 65, 76, 68, 21, 191, 241, 236, 148, 123, 0,
            117, 226, 132, 199, 220, 249, 105, 68, 218, 45, 248, 229, 104, 106,
            172, 219, 254, 141, 225, 65, 209, 175, 70, 179,
        ]);
