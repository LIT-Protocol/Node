...
describe("ReleaseRegister", function () {
    ...
    describe("init creator", async () => {
        context("having a valid role and called the first time", async () => {
            beforeEach(async () => {
                registerContract = await ethers.deployContract(
                    "ReleaseRegister",
                    [ENV_STAGING]
                );
    ...
    describe("create release", async () => {
        context("when no existing release", async () => {
            ...
            it("creates a release", async () => {
                ...
                const RELEASE_OPTION_SSH =
                    await registerContract.RELEASE_OPTION_SSH();
                ...
                expect((release.options & RELEASE_OPTION_USERS) !== 0).equals(
                    true
                );
                expect((release.options & RELEASE_OPTION_SSH) !== 0).equals(
                    true
                );
                expect(release.id_key_digest).equals(
                    TEST_RELEASE_ID_KEY_DIGEST
                );
            ...
            it("creates a release with specific date", async () => {
                ...
                expect(
                    (release.options &
                        (await registerContract.RELEASE_OPTION_RO())) !==
                        0
                ).equals(true);
                expect(release.id_key_digest).equals(
                    TEST_RELEASE_ID_KEY_DIGEST
    ...
    describe("set release status", async () => {
        context("with Active status and release", async () => {
            ...
            it("will set the status", async () => {
                ...
                await registerContract.setReleaseStatus(
                    releaseId,
                    STATUS_ACTIVE
                );
            });
        ...
        context("without DEACTIVATOR role", async () => {
            ...
            it("produces an error", async () => {
                ...
                await expect(
                    registerContract.setReleaseStatus(
                        releaseId,
                        STATUS_DISABLED
                    )
                ).to.be.revertedWithCustomError(
