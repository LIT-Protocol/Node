...
import { backupRecovery } from "../typechain-types/contracts/lit-node";
...
export async function setContractResolver(
    ...
    env: Environment,
    {
        ...
        domainWalletRegistryContract?: DomainWalletRegistry;
        domainWalletOracleContract?: DomainWalletOracle;
    ...
    if (domainWalletOracleContract) {
        await contractResolver.setContract(
            await contractResolver.DOMAIN_WALLET_ORACLE(),
            env,
            await domainWalletOracleContract.getAddress()
        );
    }
...
export async function setupStakingWithValidatorsAndAdvance(
    ...
    for (let i = 0; i < options.numValidators; i++) {
        ...
        const ipAddress = ip2int(options.ipAddress);
        const port = options.startingPort + i + 1;
